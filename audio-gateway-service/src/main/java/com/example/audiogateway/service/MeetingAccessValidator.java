...
public interface MeetingAccessValidator {
    ...
    record Decision(boolean allowed, HttpStatus status, String message, boolean retryable) {

        public static Decision granted() {
            return new Decision(true, HttpStatus.OK, "allowed", false);
        }

        public static Decision forbidden(final String message) {
            return new Decision(false, HttpStatus.FORBIDDEN, message, false);
        }

        public static Decision unavailable(final String message) {
            return new Decision(false, HttpStatus.SERVICE_UNAVAILABLE, message, true);
        }
    }
}
