...
class NotificationDeliveryQueueTest {
    ...
    @BeforeEach void setup() {
        ...
        when(repository.lockNotificationSource(id)).thenReturn(Optional.of(UUID.randomUUID()));
    }
    @Test void httpFailureChangesOnlyNotificationState() {
        ...
        verify(repository, times(2)).lockNotificationSource(id);
        verify(repository, times(2)).notificationErasureRequested(id);
    ...
    @Test void successfulRetryCompletesIndependentJob() {
        ...
        verify(repository).lockNotificationSource(id); verify(repository).notificationErasureRequested(id);
        verifyNoMoreInteractions(repository);
    }
