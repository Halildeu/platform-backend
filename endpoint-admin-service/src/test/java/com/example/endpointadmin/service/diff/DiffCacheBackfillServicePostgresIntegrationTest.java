...
@Import({DiffCacheBackfillService.class, DiffCacheService.class,
...
class DiffCacheBackfillServicePostgresIntegrationTest {
    ...
    void backfillTenant_freshCacheRowCreated_software() {
        ...
        insertSoftwareHistory(tenant, device, t1, t1);
        insertSoftwareHistory(tenant, device, t2, t2);
        ...
        assertThat(result.skippedStale()).isEqualTo(0L);
    ...
    @Test
    void backfillTenant_idempotentSecondSweep_skippedStale() {
        UUID tenant = UUID.randomUUID();
        UUID device = insertDevice(tenant);
        Instant t1 = Instant.parse("2026-06-02T10:00:00Z");
        Instant t2 = Instant.parse("2026-06-02T10:01:00Z");
        ...
        assertThat(second.skippedStale()).isEqualTo(1L);
    ...
    void backfillBatch_explicitDeviceIds_scopesCorrectly() {
        UUID tenant = UUID.randomUUID();
        UUID deviceA = insertDevice(tenant);
        UUID deviceB = insertDevice(tenant);
        UUID deviceC = insertDevice(tenant);
        Instant t = Instant.parse("2026-06-02T10:00:00Z");
        for (UUID d : List.of(deviceA, deviceB, deviceC)) {
    ...
    void backfillTenant_outdated_freshCacheRowCreated() {
        ...
        UUID device = insertDevice(tenant);
        Instant t = Instant.parse("2026-06-02T10:00:00Z");
        insertOutdatedSnapshot(tenant, device, t, t);
        insertOutdatedSnapshot(tenant, device, t.plusSeconds(60), t.plusSeconds(60));
    ...
    void backfillTenant_tenantBoundary_otherTenantUntouched() {
        ...
        Instant t = Instant.parse("2026-06-02T10:00:00Z");
        insertSoftwareHistory(tenantA, deviceA, t, t);
        insertSoftwareHistory(tenantA, deviceA, t.plusSeconds(60), t.plusSeconds(60));
        insertSoftwareHistory(tenantB, deviceB, t, t);
        insertSoftwareHistory(tenantB, deviceB, t.plusSeconds(60), t.plusSeconds(60));
