...
class TpmNonceStoreTest {
    ...
    private final String ekPub = "ekpub-sha256";
    private final String ekCert = "ekcert-sha256";

    @Test
    void consume_returnsBoundValuesExactlyOnce() {
        MutableClock clock = new MutableClock(T0);
        InMemoryTpmNonceStore store = new InMemoryTpmNonceStore(clock);
        store.issue("n1", "tok|tenant|dev", nonce, secret, akName, ekPub, ekCert, T0.plusSeconds(300));
        ...
        assertThat(first.get().ekPubSha256()).as("L1-bound EK pub digest round-trips (P0-1)").isEqualTo(ekPub);
        assertThat(first.get().ekCertSha256()).as("L1-bound EK cert digest round-trips (P0-1)").isEqualTo(ekCert);
    ...
    void consume_failsWhenExpired() {
        MutableClock clock = new MutableClock(T0);
        InMemoryTpmNonceStore store = new InMemoryTpmNonceStore(clock);
        store.issue("n1", "scope", nonce, secret, akName, ekPub, ekCert, T0.plusSeconds(300));
    ...
    void consume_scopeMismatch_doesNotBurnTheEntry() {
        MutableClock clock = new MutableClock(T0);
        InMemoryTpmNonceStore store = new InMemoryTpmNonceStore(clock);
        store.issue("n1", "correct-scope", nonce, secret, akName, ekPub, ekCert, T0.plusSeconds(300));
    ...
    void evictExpired_removesOnlyExpired() {
        MutableClock clock = new MutableClock(T0);
        InMemoryTpmNonceStore store = new InMemoryTpmNonceStore(clock);
        store.issue("old", "s", nonce, secret, akName, ekPub, ekCert, T0.plusSeconds(60));
        store.issue("new", "s", nonce, secret, akName, ekPub, ekCert, T0.plusSeconds(600));
