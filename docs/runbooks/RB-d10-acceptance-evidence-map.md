...
| O2d | Broker negative: **self-approval deny** (approver≠requester) | ✅ + 🟡 | ✅ **raw** approver≠requester invariant + negatif test: `RemoteSessionAuthz` + `RemoteSessionNegativeTest` (self-approval deny). **🟡 GAP:** alias/proxy/service-account **canonicalization** (canonical identity'e çöz → sonra approver≠requester) — bkz. E10 (OpenFGA bunu enforce edemez → app-level). |
...
- **✅ agent-evidence DONE (tam/parça):** O1, O2a, O2b, **O2d-raw** (approver≠requester), **O3** (state-machine), E2, E3-part, **E8** (gitops manifest scaffold), E7-broker-side, E9-part (FIDO2/MFA/bearer-header/tenant-auth), E4-primitives, **E11-design-time-SIM-mostly** + E1-kill-mekaniği. Bridge suite **271 test** + B2/B1/redteam-runbook bu kanıtın gövdesi.
- **🟡 agent-completable GAP (daraldı — yazılabilir, owner-gated DEĞİL):** **E10** (dual-control **alias/proxy/SA canonicalization** + approval-fatigue + broker/service flow enforcement — en net + merkezi), E9 (ws origin/CSRF + per-channel nonce — küçük net slice), O2c (broker↔jti **integration** neg-test), E4 (integrated operator/broker wire-response **timing/oracle regression**), E1 (heartbeat revoke→kill-**SLO** neg-test), E11 (eksik SIM kalmışsa daralt).
...
## Sıradaki agent-completable D10 slice önceliği (düzeltilmiş)

1. **E10 dual-control canonicalization** (alias/proxy/SA → canonical identity + approver≠requester enforce-on-canonical + approval-fatigue limit) — raw deny mevcut, **canonicalization+integration** eksik; güvenlik semantiği merkezi.
2. **E9 ws origin/CSRF + per-channel nonce** (operator REST — küçük, net agent slice).
3. **O2c/E4/E1 integration neg-testleri** (broker↔jti expired-replay tek-akış + timing/oracle regression + revoke→kill-SLO).
