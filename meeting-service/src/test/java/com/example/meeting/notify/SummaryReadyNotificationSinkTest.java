...
class SummaryReadyNotificationSinkTest {
    ...
    @Test void partialAudienceFailureThenNewSinkReplaysAcceptedIntentUnchanged() {
        var recipients = mock(MeetingReadyRecipients.class);
        var sender = mock(HttpAssignmentNotificationSink.class);
        var id = UUID.randomUUID(); var org = UUID.randomUUID();
        var event = new MeetingEventMessage("occurrence", "meeting.summary.ready", "meeting", id, 1, id, org, org, "{}");
        when(recipients.resolve(id, org, org)).thenReturn(List.of(1L, 2L), List.of(1L, 2L, 3L));
        when(sender.intent(eq(event), anyLong())).thenAnswer(call -> new LinkedHashMap<String,Object>());
        doNothing().doThrow(new IllegalStateException("second recipient offline")).doNothing().when(sender).submit(any());
        assertThatThrownBy(() -> new SummaryReadyNotificationSink(recipients, sender).deliver(event))
                .isInstanceOf(IllegalStateException.class);
        new SummaryReadyNotificationSink(recipients, sender).deliver(event);
        ArgumentCaptor<Map<String,Object>> capture = ArgumentCaptor.forClass(Map.class);
        verify(sender, times(5)).submit(capture.capture());
        var bodies = capture.getAllValues();
        assertThat(bodies.get(0)).isEqualTo(bodies.get(2));
        assertThat(bodies.get(1)).isEqualTo(bodies.get(3));
        assertThat(bodies.get(4).get("idempotencyKey")).isEqualTo("occurrence|native-ready|3");
    }
}
