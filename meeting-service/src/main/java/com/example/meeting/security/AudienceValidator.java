...
/**
 * JWT audience / authorized-party validator. Copied verbatim from
 * endpoint-admin-service so the audience contract is identical across
 * services: a token is accepted when its {@code aud} contains one of the
 * expected audiences, or its {@code azp}/{@code client_id} is in the
 * allowed-client-id set.
 */
